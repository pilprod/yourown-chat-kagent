...
Текущий статус: **только оценка совместимости**. Зафиксированная версия пока
не удовлетворяет управляемому профилю: отсутствуют доверенная авторизация,
продолжение HITL через публичный шлюз и обязательный маршрут MCP через Tool
Gateway. Репозиторий не владеет продуктивным релизом, не публикует образы и не
разворачивает kagent.
